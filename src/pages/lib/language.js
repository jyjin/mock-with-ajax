...
// 多语言查询组件
const queryLanguage = (_module, otherCode = [], language) => {
  const lang = localStorage.getItem('languageUserSelect')
  if (language[_module] != null) {
  ...
  return new Promise(resolve => {
    Ajax.post(url, requestCode).then((_lang) => {
      if (!_lang) _lang = {}
      language[_module] = prefixRemove(allLanglist, _lang, _module, devicePrefix)
